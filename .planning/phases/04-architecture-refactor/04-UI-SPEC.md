...
note: Preservation contract — Phase 4 is a refactor. Visual output must not change.
---

# Phase 4 — UI Design Contract

> Visual and interaction contract for the architecture refactor.
> Generated by gsd-ui-researcher. This phase is a non-visual refactor — the contract
> documents what MUST stay the same when FogWisp, Leaf, and scene components are
> extracted. Executor and auditor use this as the locked reference.
...
All values are multiples of 4 except the `leafTouchArea` padding (15px) which is a touch target exception.
...
Exceptions:
- `uiZone` paddingHorizontal: 20px — existing value, preserve as-is
- `leafTouchArea` padding: 15px — touch target exception, intentionally between sm and md
- `speechBubble` paddingVertical: 12px — existing value, preserve as-is
- `speechBubble` paddingHorizontal: 20px — existing value, preserve as-is
